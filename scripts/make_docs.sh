...
usage() {
  ...
  echo ""
  echo "Build Pytext documentation. Must be executed from root of Pytext repository."
  echo ""
...
if [[ $ONLY_DOCUSAURUS == false ]]; then
  ...
  cwd=$(pwd)
  python3 scripts/parse_sphinx.py -i "${cwd}/pytext/docs/build/html/" -o "${cwd}/website/pages/api/"

  SPHINX_JS_DIR='pytext/docs/build/html/_static/'
  DOCUSAURUS_JS_DIR='website/static/js/'

  mkdir -p $DOCUSAURUS_JS_DIR

  # move JS files from pytext/docs/build/html/_static/*:
  ...
  cp "pytext/docs/build/html/searchindex.js" "${DOCUSAURUS_JS_DIR}searchindex.js"

  # copy module sources
  cp -r "pytext/docs/build/html/_sources/" "website/static/_sphinx-sources/"
